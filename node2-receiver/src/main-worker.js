...
async function main() {
  ...
  const networkName = process.argv[2] || deployment.network || process.env.DEFAULT_NETWORK || 'monad';
  const networkConfig = getNetworkConfig(networkName);

  console.log(`📄 Contract: ${deployment.address}`);
  console.log(`🌐 Network: ${networkConfig.name} (${networkConfig.chainId})`);
  console.log(`⏰ Polling Interval: ${process.env.POLLING_INTERVAL || 5000}ms\n`);
  ...
  try {
    ...
    worker.on('taskCompleted', ({ taskId, result, transactionHash, blockNumber }) => {
    ...
    setInterval(async () => {
      ...
      if (stats) {
        ...
        console.log(`💰 ETH Balance: ${stats.ethBalance}`);
        console.log(`💎 Earned: ${stats.earnedBalance} ETH`);
