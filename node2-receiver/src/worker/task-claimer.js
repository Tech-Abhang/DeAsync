...
export class TaskClaimer extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      console.log(
        `🔗 Worker connected to ${network.name || "Monad"} (Chain ID: ${
          network.chainId
        })`
      );
  ...
  async startPolling() {
    ...
    console.log(
      `🚀 Starting ${this.workerName} - Polling every ${this.pollingInterval}ms`
    );
    this.isRunning = true;
      ...
      for (const task of latestTasks) {
        ...
        if (
          ...
          (task.worker !== ethers.ZeroAddress &&
            task.worker !== this.wallet.address)
        ) {
        ...
        if (task.worker === ethers.ZeroAddress && !task.completed) {
          console.log(
            `🎯 Found unclaimed task #${taskId} - attempting to claim`
          );
  ...
  async attemptClaimTask(taskId) {
    try {
      ...
      if (balanceETH < 0.005) {
        console.error(
          `💰 Insufficient balance: ${balanceETH.toFixed(
            6
          )} MONAD. Skipping claim.`
        );
        return;
      }
      ...
      console.log(
        `📋 Claiming task #${taskId} with nonce ${nonce} and gas ${ethers.formatUnits(
          gasPrice,
          "gwei"
        )} gwei`
      );
      ...
      console.log(
        `✅ Task #${taskId} claimed successfully in block ${receipt.blockNumber}`
      );
      ...
      if (
        ...
        console.log(
          `🔢 Task #${taskId}: Nonce issue detected - ${error.message}`
        );
  ...
  async submitTaskResult(taskId, result) {
    try {
      ...
      const receipt = await tx.wait();
      console.log(
        `✅ Result submitted for task #${taskId} in block ${receipt.blockNumber}`
      );
      ...
      if (
        ...
        console.log(
          `🔢 Task #${taskId} result submission: Nonce issue - ${error.message}`
        );
        await this._refreshNonce();

        // Keep task in claimed tasks for retry on next cycle
        console.log(
          `⏱️ Will retry result submission for task #${taskId} in next round`
        );
        return;
      }

      console.error(
        `❌ Failed to submit result for task #${taskId}: ${error.message}`
      );
    }
  }
